...
def extract_melody_line(midi_data, time_resolution=0.1):
    """Extract the main melody line from polyphonic MIDI data"""
    ...
    for i, time_point in enumerate(time_points):
        ...
        if active_notes:
            best_note = max(active_notes, key=lambda n: n['pitch'] + (n['velocity'] / 127.0) * 12)
            
            if not melody_notes or melody_notes[-1]['pitch'] != best_note['pitch']:
                melody_notes.append({
    ...
    for i in range(len(melody_notes) - 1):
        melody_notes[i]['duration'] = melody_notes[i + 1]['start'] - melody_notes[i]['start']
    
    if melody_notes:
        melody_notes[-1]['duration'] = time_resolution * 4
...
def main():
    """Main function with multiple Basic-Pitch options"""
    print("=== ENHANCED VIOLIN MELODY CONVERTER ===")
    print("Multiple Basic-Pitch integration options:")
    print("1. Hugging Face model (if available)")
    print("2. Original basic-pitch package")
    print("3. Direct inference method")
    print()
    ...
    # Step 2: Try different Basic-Pitch methods
    midi_file = None
    
    # Option 1: Try Hugging Face version
    if HF_AVAILABLE:
        print("\n[+] Trying Hugging Face Basic-Pitch...")
        midi_file = run_basic_pitch_hf(CLEAN_FILE, OUTPUT_DIR)
    ...
    if not midi_file and BASIC_PITCH_AVAILABLE:
        ...
        midi_file = run_basic_pitch_direct_inference(CLEAN_FILE, OUTPUT_DIR)
    ...
    if not midi_file and BASIC_PITCH_AVAILABLE:
        ...
        run_basic_pitch_original(CLEAN_FILE, OUTPUT_DIR)
        midi_file = find_midi_file(OUTPUT_DIR, CLEAN_FILE)
